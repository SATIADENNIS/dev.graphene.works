...
If an order is created and paid in a non-GPH asset, the fee is
implicitly exchange into GPH to pay the network fee. However, if the
order is canceled, 90% of the fee will be returned as GPH. The result
...
price of your asset. As a consequence, paying fees in GPH should always
be cheaper.
...
pay for the transaction fees without the need to hold GPH. Any
...
GPH to cover the network fee. If the asset's fee pool is funded, the
fees can be payed in the native UIA instead of GPH.

.. note:: The core exchange rate at which a fee can be exchanged into
          GPH may differ from the actual market valuation of the asset.
...
required to hold GPH for the fee.
...
In Graphene, You pay the fee upon **receiving an asset**. Hence, one
