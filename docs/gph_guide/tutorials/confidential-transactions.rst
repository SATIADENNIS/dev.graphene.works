...
**How to use the CLI wallet to perform confidential transfers in Graphene**


This tutorial shows how to use the CLI wallet to perform confidential transfers
in Graphene. A confidential transfer is one that hides both the amount being
...
consult the `readme file <https://github.com/bitshares/bitshares-2>`_ on github or the Graphene wiki. I will be using
...
Graphene. Instead a blind account is nothing more than a labeled public key.
...
"brainkey" is effectively the private key used by your account. All Graphene
...
::
    ...
    >>> transfer_to_blind "peters-public-registered-account" GPH [[Alice-is-Blind,100]] true
    3369305ms th_a       wallet.cpp:3794               transfer_to_blind    ] to_amounts: [["Alice-is-Blind","100"]]
    peters-public-registered-account sent 100 GPH to 1 blinded balance fee: 40 GPH
    100 GPH to  Alice-is-Blind
...
The above 2 steps transmit GPH assets from a public, registered account named
...
::
    ...
    100 GPH  peter  =>  alice   "from Peter"
...
feature as currently implemented in Graphene *DOES NOT POSSESS* the property of
...
GPH balance. We will create a label to refer to Bob's confidential account
(bobby) and transfer some GPH assets from alice to bobby. Note that the process
...
newly created "bobby" account, and the blind_transfer command fee was 15 GPH,
which sent 80 GPH of the balance (100 GPH was transferred to the alice account
in Part 1) to the bobby confidential account and provided 2 balance receipts:
the first for 5 GPH coming back to the alice account as returned change
(leftover funds), and the second which is the receipt for the 80 GPH being sent
...
to import the 5 GPH change back into the alice account, at least that is taken
care of. Also important to note is as far as the outside world can see alice
sent some amount less than 100 GPH to two new outputs, one of which is the
...
In this final step of our round-trip process we will transfer some of the GPH
...
::
    ...
    100 GPH  alice  =>  bobby   "from Alice"
...
(`bobby`) is a key label, but the second (`peter`) is a registered public Graphene
account.

::

    >>> transfer_from_blind bobby peter 50 GPH true
...
show (publicly) that it has received 50 GPH from an "unknown" source.
...
::
    ...
    >>> transfer_to_blind peters-public-registered-account GPH [[alice,800],[alice,2000],[bobby,2000]] true
    peters-registered-account sent 4800 GPH to 3 blinded balances fee: 40 GPH
    800 GPH to  alice
...
GPH to three different blinded destinations. Note that although a sum of 2800 GPH were sent to
...
or who is in control of each one, only that together they all add up to 4800 GPH.


Summary
^^^^^^^

We covered several steps and several operations with Confidential Transactions, a.k.a. "blind" transfers, in Graphene, using the CLI (command line) wallet.  A brief summary of the wallet commands:
