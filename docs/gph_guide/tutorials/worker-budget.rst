...
workers where reserve fund is how many GPH are currently not distributed (see
the `source code`_).
...
GPH are available to be distributed and how many GPH will be returned to the
...
(i.e., 1.5 GPH per block is budgeted, so budget remaining is

::

    1.5 GPH * (number of blocks left in maintenance period+blocks missed by witnesses))
