...
***************************************************************
How to prepare Graphene Exchange Guide (Single Node Edition)
***************************************************************

The purpose of this article is to assist third-party exchanges to go online for Graphene transactions.
...
The Graphene uses the DPOS consensus mechanism to vote for the block forge by the person holding the GPH. The standard block interval is 3 seconds.

**1.2 Account**

1) In Graphene, funds are stored in an account, unlike Bitcoin where there is an address. For the exchange, it is necessary to open an account for users to deposit.
...
- In the light wallet account page, a number is displayed below the account number. This number is the built-in ID of the account in the Graphene system, which will be used below.
...
1) There are various assets in the Graphene system, of which the core asset is the GPH. The approach of the exchange to other assets in the Graphene system is similar to that of the GPH.
...
To start-off the Graphene system, you need to run these programs: normal node ``witness_node``, command line wallet ``cli_wallet``.

**3.1 Architecture Description**

- The ``witness_node`` is connected to the Graphene network in a P2P manner, receives the latest block from the network, and broadcasts the locally signed transaction packet to the network.
...
The compiled Windows executable is available for download on Github, at https://github.com/graphene-blockchain/graphene-core/releases/latest ,

The file is `Graphene-Core-2.0.xxxxxx-x64-cli-tools.zip` and it can be unzipped. It contains three exe files and two dll files. Here is the :ref:`installation guide: CLI-Wallet on Windows (x64) <cli-tool>`
...
3) The following parameters will reduce the memory required for operation. The principle is that the historical transaction record index of the GPH built-in transaction engine is not saved because the exchange does not normally use this data.::
...
* The amount is **{"amount":467116432,"asset_id":"1.3.0"}** format, where
  ...
  * The ``asset_id`` can be found by the ``get_asset`` command. The ``asset_id`` of the GPH is 1.3.0. Other assets have other ids.
  * amount is the value after the decimal point is removed. For example, GPH is `5 decimal` places. In the above example, it is actually 4671.16432 GPH.
...
The possibility of using Bitcoin and others to use the confirmation number to reduce the probability of a fallback in the transaction is different. In the Graphene, the **unreturnable block** number can be used to determine whether the transaction can be rolled back.
...
.. code-block:: json
	...
	{
	"memo":"",
	"description":"Transfer 1 GPH from a to b -- Unlock wallet to see memo. (Fee: 0.22941 GPH)",
	"op":{
	"id":"1.11.1234567",
	"op":[
	 0,
...
Use the ``transfer2`` command to send a withdrawal transaction. Such as::
	...
	unlocked >>> transfer2 withdrawal-account to-account 100 GPH "some memo"
...
1) If the currency is GPH, the number of decimal places is up to 5 digits. If it is other assets, you can view the decimal places of the asset with the ``precision`` field with the ``get_asset`` command.
...
Unlike Bitcoin, there is a timeout in the GPH transaction.
...
* **Multi-signature**: Graphene natively supports account-level multi-signature, and there is a proposal-approval mechanism that can initiate multi-signature requests online and then confirm the completion of multi-signature transactions.  Read about: :ref:`bts-multi-sign`
...
This information originates [abitmore/bts-cn-docs](https://github.com/abitmore/bts-cn-docs/blob/master/GPH%E4%BA%A4%E6%98%93%E6%89%80%E5%AF%B9%E6%8E%A5%E6%8C%87%E5%8D%97%EF%BC%88%E5%8D%95%E8%8A%82%E7%82%B9%E7%89%88%EF%BC%89.txt) repository.
