...
You can create a Worker Proposal by using Graphene UI form. Go to *Voting* page. There are three tabs: "Witnesses", "Committee", and "Workers". In the "Worker" tab, you will find *CREATE A NEW WORKER* button.
...
An `owner_account` is creating a one day worker starting Oct 28 and will get paid 1 GPH/day (vesting in 1 day, 1 GPH = 100,000 ‘satoshi’) to make an android app. The first command won’t broadcast, this will just check::
...
The variable ``pay_vesting_period_days`` is the integer number of days you set for vesting. Some people don’t want workers to withdraw and sell large sums of GPH immediately, as it puts sell pressure on GPH. Also, if you require vesting, you have “skin in the game” and thus an incentive to improve GPH value. Pay is pay per day (not hour or maintenance period) and is in units of 1/100,000 GPH (the precision of GPH)
...
You can check the Worker Proposals by Graphene UI form. Go to *Voting* page. There are three tabs: "Witnesses", "Committee", and "Workers". In the "Worker" tab, you will find all Worker Proposals list.
...
So the daily budget is::
    ...
    worker_budget*24 = 1340913100 * 24 = 32181914400 (or 321,8191.44 GPH)

There is currently a maximum daily worker pay of 500k GPH, and this can be found using the `get_global_properties` command in the cli_wallet.
...
is allocated for witnesses and workers. The reserve fund is maximum number of GPH available less those currently in circulation ([source](https://github.com/cryptonomex/graphene/blob/f85dec1c23f6bf9259ad9f15311b2e4aac4f9d44/libraries/chain/include/graphene/chain/config.hpp))

Every hour the total available reserve fund is calculated by finding how many GPH are available to be distributed and how many GPH will be returned to the reserve fund (i.e., “burnt”) during the next maintenance interval.

First find how many GPH have not been distributed::

    >>> from_initial_reserve = max_supply - current supply of GPH
...
Modify it by adding the accumulated fees and witness budget remaining (i.e., 1.5 GPH per block is budgeted, so budget remaining is 1.5 GPH * (number of blocks left in maintenance period+blocks missed by witnesses)) in this maintenance cycle (they will be added to the “reserve fund” permanently at maintenance)::
