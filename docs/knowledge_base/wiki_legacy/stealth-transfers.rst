...
.. code-block:: json
    ...
    >>> transfer_to_blind alicepub GPH [[alice,1000],[alice,2000]] true
    alicepub sent 3000 GPH to 2 blinded balances fee: 15 GPH
    1000 GPH to  alice
...
In this case the only thing the public sees is that account 'alicepub' sent 3000 GPH to two different places. The outside world has no idea how much is in each output, only that they add up to 3000 GPH.

Viewing Blind Balances
^^^^^^^^^^^^^^^^^^^^^^^^^^^

Alice can verify that she has 3000 GPH in blinded balances by using the command:
...
Alice can now transfer to "Bob" which she has labeled 'bobby' in her wallet via the following command::
    ...
    >>> blind_transfer alice bobby 500 GPH true
...
The output shows that 500 GPH was sent to bobby and 485 GPH sent back to alice as change after paying a 15 GPH fee. If we check the balance of alice we will see::

    >>> get_blind_balances alice
    2485 GPH

The outside world only knows that alice sent some amount less than 3000 GPH to two new outputs.
...
.. code-block:: json
    ...
    ====================================================================================
    19 seconds ago  500 GPH  alice  =>  bob  memo
...
Eventually every blind balance needs to convert back to a public balance which can be achieved with the following command::
    ...
    >>> transfer_from_blind alice alicepub 1000 GPH true
    { ... }
