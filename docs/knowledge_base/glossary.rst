...
GPH Holder
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
- "shareholder / stakeholders" --> "GPH Holder"
...
Genesis Block (in Graphene)
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
- Graphene 2.0 will be initialized with what is called a Genesis Block. That genesis block will be constructed from the balances of Graphene 1.0.
...
- InGraphene, a seed node is a node that accept incoming P2P connection. Its address is hard coded in the program, so when a new node starts, it will connect to the seed nodes by default. Every node (including seed nodes) tells the connected nodes where other nodes are, so all nodes can connect to each other.
...
unit test
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
- InGraphene-Core unit test files locate in a /test/ folder. They are meant to ensure that the code behaves the right way.
