...
In contrast to most cryptocurrency wallets, the Graphene 2.0 has a different
...
the Graphene 2.0 client. Furthermore, we will briefly introduce the API and
...
On the Graphene blockchains there are no addresses, but objects identified by a
...
The Graphene blockchain users are requires to register each account with a unique username and a public key on the blockchain. The blockchain assigns an incremental user id and offers to resolve the name-to-id pair.
...
- A programmatic description of all fields can be found in the `sources <https://github.com/graphene-blockchain/graphene-core/blob/master/libraries/protocol/include/graphene/protocol/types.hpp>`_
