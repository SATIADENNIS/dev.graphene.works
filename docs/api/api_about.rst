...
Graphene APIs are available by using Remote Procedure Calls (RPC) and WebSocket Calls & Notifications. All API calls are formatted in JSON and return JSON only. We look into the APIs' every feature. Graphene-Core API references locate in the `Doxygen documentation. <https://bitshares.org/doxygen/index.html>`_  Also, you can find the APIs' classes, components, and elements information looking into "Graphene - Namespaces" section.
...
* Here is the latest list of `Graphene Public Full Nodes (apiConfig) <https://github.com/graphene-blockchain/graphene-ui/blob/staging/app/api/apiConfig.js>`_
