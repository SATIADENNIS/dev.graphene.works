...
.. code-block:: cpp
	...
	//   ilog("Processing payroll! Available budget is ${b}", ("b", budget));
	   ...
	   for( uint32_t i = 0; i < active_workers.size() && budget > 0; ++i )
	   {
		  ...
		  //       Since it's not the case on Graphene mainnet, we're not using a check here.
...
.. code-block:: cpp
	 ...
	 * https://github.com/graphene-blockchain/graphene-core/issues/868
	 *
	 ...
	 * https://github.com/graphene-blockchain/graphene-core/issues/890
	 *
	...
	void process_hf_868_890( database& db, bool skip_check_call_orders )
	{
	   ...
	   for( auto asset_itr = asset_idx.lower_bound(true); asset_itr != asset_idx.end(); ++asset_itr )
	   {
		  ...
		  // https://github.com/graphene-blockchain/graphene-core/pull/832#issuecomment-384112633
		  ...
		  // always update the median feed due to https://github.com/graphene-blockchain/graphene-core/issues/890
...
.. code-block:: cpp
	 ...
	 * https://github.com/graphene-blockchain/graphene-core/issues/935
	 *
	 * @param db the database
	 */
	// TODO: for better performance, this function can be removed if it actually updated nothing at hf time.
...
.. code-block:: cpp
	...
	bool database::apply_order(const limit_order_object& new_order_object, bool allow_black_swan)
	{
	   ...
	   //       https://github.com/graphene-blockchain/graphene-core/issues/606 .
...
.. code-block:: cpp
	...
	{ try {
		...
		while( !check_for_blackswan( mia, enable_black_swan, &bitasset ) && call_itr != call_end )
		{
		   ...
		   // Old rule: margin calls can only buy high https://github.com/graphene-blockchain/graphene-core/issues/606
