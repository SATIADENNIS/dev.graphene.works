...
- 1a) Install and build it from Graphene-Core
  - Graphene offers you to install Graphene-Core into different platforms; Ubuntu (x64), OSx, and Windows.  Please refer :ref:`installation-guide`.

- 1b) Use the :ref:`CLI-Wallet tools for Windows (x64)`
  - This option you do not need to install BitShates Core. To obtain the tool, go to a `Graphene release page <https://github.com/graphene-blockchain/graphene-core/releases>`_ and download the Graphene-Core-\*-x64-cli-tools.zip.
...
We use the public API node of OpenLedger ``wss://node.market.rudex.org`` and connect via secured WebSocket connection:
